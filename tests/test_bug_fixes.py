...
from monster import create_monster_for_level, Dragon
...
def test_headlamp_fov_calculation():
    ...
    assert fov_with_headlamp == 11, f"Expected FOV 11 with headlamp, got {fov_with_headlamp}"
...
def test_spiked_armor_stats():
    ...
    assert spiked_armor.defense_bonus == 4, f"Expected defense bonus 4, got {spiked_armor.defense_bonus}"
    ...
    expected_defense = base_defense + 4  # base + spiked armor
...
def test_dragon_spawning_levels():
    """Test that dragons only spawn on level 10, not level 9."""
    ...
    assert 'Dragon' not in level_9_monsters, f"Dragons found on level 9: {level_9_monsters}"
    ...
    assert level_10_monsters == {'Dragon'}, f"Expected only Dragons on level 10, got: {level_10_monsters}"
    print(f"Level 10 monsters: {level_10_monsters}")
    
    print("✓ Dragons only spawn on level 10")
...
def test_level_progression_monsters():
    ...
    for level in range(1, 11):
        ...
        # Verify no dragons appear before level 10
        if level < 10:
            assert 'Dragon' not in monsters, f"Dragon found on level {level}"
        else:
            assert monsters == {'Dragon'}, f"Expected only Dragons on level 10, got {monsters}"
...
def test_game_integration_fov_update():
    ...
    # Equip headlamp through game system
    game.equip_item(headlamp)
    
    # FOV should now include bonus
    final_fov = player.get_total_fov()
    assert final_fov == 11, f"Expected FOV 11 after equipping headlamp, got {final_fov}"
